...
(() => {
  ...
  // ───────────────────────── Paramètres ─────────────────────────
  function testChannel(channel, p) {
    const defaultTo = channel === "EMAIL" ? state.user.email || "" : p.telephone_pasteur || "";
    const to = prompt(channel === "EMAIL" ? "Adresse e-mail de test :" : `Numéro de test au format international (+33…) :`, defaultTo);
    if (to === null) return;
    safe(async () => {
      const r = await api("/parametres/test-envoi", { method: "POST", body: { channel, to } });
      modal(`${r.ok ? "✅" : "❌"} Test ${r.canal} via ${r.fournisseur}`, el("div", {},
        el("dl", { class: "kv" }, el("dt", {}, "Destinataire"), el("dd", { class: "mono" }, r.destinataire), el("dt", {}, "Résultat"), el("dd", {}, r.ok ? (r.live ? "Envoi accepté par le fournisseur" : "Simulé (mode démonstration)") : "Échec"), r.erreur ? el("dt", {}, "Erreur du fournisseur") : "", r.erreur ? el("dd", { class: "mono small" }, r.erreur) : "", r.identifiant ? el("dt", {}, "Identifiant") : "", r.identifiant ? el("dd", { class: "mono small" }, r.identifiant) : ""),
        r.conseil ? el("p", { class: "small", style: "margin-top:10px" }, "💡 ", r.conseil) : ""));
    });
  }
